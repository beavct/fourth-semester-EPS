  ...
  IfC cond b1 b2 ->
    case eval cond env of
      NumV num ->
       ...
       -- -> error "ERRO eval IfC: condição não é um número"
      -- MODIFICAÇÃO
      --BoolV bool ->
      --    if bool == "true"
      --      then eval b1 env

      BoolVTrue bool -> eval b1 env
      BoolVFalse bool -> eval b2 env
      _ -> error "ERRO eval IfC: condição não é um número ou booleano"
